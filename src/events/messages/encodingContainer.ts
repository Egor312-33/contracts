...
export function isVideoContainer(
  container: string,
): container is VideoContainer {
  return Object.values(VideoContainer).includes(container as VideoContainer);
}

export function isEncodingContainer(
  container: string | undefined,
): container is EncodingContainer {
  return Object.values(EncodingContainer).includes(
    container as EncodingContainer,
  );
}

export function mapVideoContainerToContentType(
...
export function mapEncodingContainerToContentType(
  container: EncodingContainer,
): string {
  const encodingContainerToContentTypeMapping: Record<
    EncodingContainer,
    string
  > = {
